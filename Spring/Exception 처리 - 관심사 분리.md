스프링을 처음 배우면서 exception에 대한 처리는 
1. `@ControllerAdvice`와 `@ExceptionHandler`를 활용한 글로벌 처리
2. 로컬 처리
이 두가지를 기준으로 생각하였었다. 

처음에는 공통 Exception처리는 꼭 필요하다고 생각하여 공통 처리를 위해 GlobalExceptionHandler에서 Exception의 종류별로 정의한 후, 각 Exception에 HttpsStatus 코드를 맞춰서 정리하였다. 
