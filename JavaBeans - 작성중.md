...
위와 같은 방식으로 생성자 자체가 없는 경우에도 `new NoDefaultConstructor()` 이 실행된다. 이는 생성자 선언을 해주지 않았을 경우 자바 컴파일러에서 자동으 기본 생성자를 생성해주기 때문이다.

위와 같이 자바의 기본 동작에서도 기본 생성자를 만들어주고 있는데, 이처럼 기본 생성자가 존재하여야 한다 라는 원칙이 세워진 이유는 다음과 같다. 

### 재사용성
기본 생성자가 있으면 특정 인자를 요구하지 않고도 Bean을 생성할 수 있어서 다양한 환경에서 쉽게 사용하는 것이 가능하다. 

### 프로퍼티 설정
JavaBeans는 기본 생성자를 통해 객체를 생성한 후, setter 메서드를 통해 프로퍼티를 설정해줄 수 있다. 이는 Bean이 복잡한 초기화 로직을 갖지 않아도 쉽게 설정될 수 있도록 해준다. 

### 도구 및 프레임워크 지원
Spring이나 JPA 등 여러 자바를 이용한 도구나 프레임워크에서는 리플렉션을 통해 객체를 생성하는데, 기본 생성자가 있는 경우에는 이러한 도구들이 객체를 쉽게 인스턴스화 처리할 수 있게 된다. 
