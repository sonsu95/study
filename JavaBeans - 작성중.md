...
그 이유를 크게 나눠보자면 3가지로 살펴볼 수 있다. 유연성, 재사용성, 프레임워크 호환성의 3가지 이다. 

#### 유연성
기본생성자를 통해 인스턴스를 생성하고, setter 메서드를 통해 인스턴스의 상태를 설정할 수 있다는 점과 인스턴스를 생성할 때 인스턴스의 상태를 고정한다는 점은 매우 큰 차이가 있다. 
개발자에게 객체의 생성과 속성 설정을 분리할 자유도를 제공한다는 점이다. 
