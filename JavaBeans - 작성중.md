...
왜 JavaBeans 규칙에서는 굳이 기본 생성자가 있는 방식을 권장하는 걸까. 

우리가 인스턴스를 활용할 때, 기본 생성자를 사용한 방식으로 인스턴스를 만들어 낸 경우, 생성된 인스턴스에 setter 메서드를 통해 직접 값을 설정할 수 있다. 인수를 받는 생성자의 경우에는 인스턴스가 생성될 때부터 해당 값들이 지정되다보니 기본 생성자를 활용하는 방식보다 유연성이 떨어진다고 볼 수 있다. 

그렇다면 왜 굳이 유연성을 강조를 하는 걸까. 

가장 먼저 고려할 수 있는 점은, 개발자의 자유도를 높일 수 있다는 점이다. 기본 생성자를 생성해두면 개발자는 초기화 매개변수 없이 언제든지 Bean 인스턴스를 생성할 수 있고, 다양한 상황에서 이를 활용할 수 있다. 하지만 이런 방식은 거꾸로 규칙으로 정하기에는 너무 자유로운 방식으로 생각되기에, 사실상 의도한 바와 다르지 않을까라는 생각이 들었다. 
