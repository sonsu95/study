...
그러나 이 패턴으,ㄴ 인스턴스의 일관성과 완전성이 보장되지 않을 수 있으며, public setter 메서드로 인해 외부에서의 무분별한 접근이 가능하여 불변성을 해친다는. 또한 멀티스레드 환경에서는 객체의 상태가 예측하기 어려울 수 있으며, 객체의 불변성을 보장하기 어렵다는 문제가 있다.
...
그런데 만약 치즈가 빠져버려 실제로는 setCheese 메서드를 실행시키지 않았다면, 생성된 cheeseBurger를 사용하는 입장에서는 알 방도가 없다. 마찬가지로 누가 치즈 대신 다른걸 추가해서 치즈버거라고 인스턴스를 생성해 두었을 경우에도 사용하는 입장에서는 알 방도가 없다. 즉, 인스턴스의 일관성과 완전성이 보장되지 않는 것이다. 
