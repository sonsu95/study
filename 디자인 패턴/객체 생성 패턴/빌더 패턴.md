...
그러나 이 패턴의 단점으로는 인스턴스의 일관성과 완전성이 보장되지 않을 수 있으며, public setter 메서드로 인해 외부에서의 무분별한 접근이 가능하다는 점이다. 또한 멀티스레드 환경에서는 객체의 상태가 예측하기 어려울 수 있으며, 객체의 불변성을 보장하기 어렵다는 문제가 있다.

예를 들어보자, 아래와 같은 햄버거라는 클래스가 정의되어 있다고 생각할 때 
...
해당 클래스를 통해 생성하고자 치즈버거라는 인스턴스를 생성하고자 한다.
...
그런데 만약 치즈가 빠져버려 실제로는 setCheese 메서드를 실행시키지 않았다면, 생성된 cheeseBurger를 사용하는 입장에서는 알 방도가 없다. 마찬가지로 누가 cheese 대신 다른걸 추가해서 cheeseBurger이라고 인스턴스를 생성해 두었을 경우에도 사용하는 입장에서는 알 방도가 없다. 즉, 인스턴스의 일관성과 완전성이 보장되지 않는 것이다. 

또한 JavaBeans 규칙에서는 setter 메서드는 public으로 선언되어야 하는데, 이같은 경우 외부에서 해당 setter에 무분별한 접근이 가능하다. 
