...
하지만 반대로 해당 객체가 완전히 생성된 상태인지의 여부를 확실하게 보장할 수 없으며, 동일하게 일관성이 결여될 수도 있다는 단점이 있다.  또한 public으로 선언된 setter 메서드를 사용하여 속성을 채워주는 방식으로 동작하다보니 외부에서의 접근에 너무 열려있다는 단점도 있다. 
