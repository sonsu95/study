

## 빌더 패턴이란?

빌더 패턴은 객체의 생성 과정에서 객체의 생성과 표현의 방식을 분리하여 다양한 종류의 인스턴스를 생성하는 디자인 패턴이다. 생성자에 들어갈 매개 변수를 메서드를 통해 하나하나 받고, 마지막에 통합하여 빌드하는 방식으로 구성된다. 

## 빌더 패턴 탄생 배경

### 점층적 생성자 패턴(Telescoping Constructor Pattern)

점층적 생성자 패턴은 객체의 생성자에서 매개변수의 수를 점차 늘려가며 여러 생성자를 제공하는 방식이다. 이 패턴은 객체를 초기화할 때 자주 사용되며, 필요한 매개변수만을 지정하여 객체를 생성할 수 있는 유연성을 제공한다. 그러나 매개변수의 수가 많아질수록 각 매개변수의 역할과 순서를 정확히 기억해야 하는 문제가 발생한다.

예를 들어, 매개변수가 두 개 또는 세 개인 경우에는 크게 문제가 되지 않지만, 매개변수가 다수인 경우에는 사용자가 각 매개변수의 순서와 역할을 정확히 알고 있어야 한다. 또한, 특정 매개변수에 값을 지정하지 않으려면 해당 매개변수를 처리할 수 있는 새로운 생성자를 추가하거나 기본값을 지정해야 할 필요가 생긴다.

즉, 점층적 생성자 패턴은 객체의 일관성과 불변성은 확보할 수 있지만, 매개변수가 많을 때 코드의 가독성과 관리가 어려워진다는 단점이 있다고 볼 수 있다.

```java
public class Hamburger {
    private int buns;
    private int patties;
    private int lettuce;
    private int tomato;
    private int cheese;
    private int bacon;
    private int pickles;
    private int onions;

    // 모든 재료를 포함하는 생성자
    public Hamburger(int buns, int patties, int lettuce, int tomato, int cheese, int bacon, int pickles, int onions) {
        this.buns = buns;
        this.patties = patties;
        this.lettuce = lettuce;
        this.tomato = tomato;
        this.cheese = cheese;
        this.bacon = bacon;
        this.pickles = pickles;
        this.onions = onions;
    }

    // 치즈버거를 위한 생성자
    public Hamburger(int buns, int patties, int lettuce, int tomato, int cheese) {
        this(buns, patties, lettuce, tomato, cheese, 0, 0, 0);
    }

    // 베이컨 치즈버거를 위한 생성자
    public Hamburger(int buns, int patties, int lettuce, int tomato, int cheese, int bacon) {
        this(buns, patties, lettuce, tomato, cheese, bacon, 0, 0);
    }

    // 빵과 베이컨만 있는 햄버거를 위한 생성자
    public Hamburger(int buns, int bacon) {
        this(buns, 0, 0, 0, 0, bacon, 0, 0);
    }
    
    // => 생성자 오버로딩 증가
}

// 매개변수가 많은 경우
Hamburger fullBurger = new Hamburger(2, 1, 2, 1, 1, 2, 3, 1);

// 선택적 매개변수 처리가 곤란한 경우
Hamburger baconOnlyBurger = new Hamburger(2, 0, 0, 0, 0, 2, 0, 0);
```

### 자바빈 패턴(JavaBeans Pattern)

자바빈 패턴(JavaBeans Pattern)은 점층적 생성자 패턴(telescoping constructor pattern)의 문제점을 해결하기 위해 고안된 객체 생성 방식이다. 이 패턴은 JavaBeans 규칙에 따라 객체를 생성하며, 기본적으로 매개변수가 없는 생성자를 사용하여 객체를 생성한 뒤, 필요한 속성은 setter 메소드를 통해 설정하는 방식으로 동작한다.이를 통해, 사용자는 필요한 속성만 설정할 수 있으며, 설정되지 않은 속성은 클래스에 정의된 기본값으로 초기화된다. 

JavaBeans 패턴에 대해서는 다음 자료를 참고

해당 패턴의 주요 특징은 유연성과 가독성이 뛰어나다는 것이다. 사용자가 객체의 상태를 단계적으로 구성할 수 있으며, 코드를 통해 어떤 속성이 설정

자바빈 패턴의 주요 특징은 유연성과 가독성이 뛰어나다는 것입니다. 사용자가 객체의 상태를 단계적으로 구성할 수 있으며, 코드를 통해 어떤 속성이 설정되었는지 명확하게 알 수 있습니다. 그러나 이 방식은 객체가 완전히 생성된 상태가 아니면 일관성이 결여될 수 있다는 단점도 가지고 있습니다. 완전한 객체의 생성을 보장하기 위해서는 추가적인 논리가 필요할 수 있습니다.


점층적 생성자 패턴에서 발생하는 문제를 해결하기 위해 만들어진 방식이 JavaBeans 규칙에 따른 객체 생성 패턴이라고 볼 수 있다. 정확히 말하자면 JavaBeans 규칙이 생겨나고, 그 규칙 중 기본 매개변수를 활용하는 방식이 존재하는 것이며, 그 방식을 흔히 JavaBeans 패턴이라고 부르는 것이다. 

이 방식은 아무런 매개변수를 받지 않는 기본 생성자를 통해 생성한 인스턴스에 setter를 통해 필드를 채워주는 방식으로 동작한다. setter로 채워주지 않은 필드의 경우에는 기본적으로 초기화된 상태로 나타난다. 

```java
import lombok.Getter;
import lombok.Setter;

@Getter
@Setter
public class Hamburger {
    private int buns;
    private int patties;
    private int lettuce;
    private int tomato;
    private int cheese;
    private int bacon;
    private int pickles;
    private int onions;
}

// 모든 재료를 포함하는 햄버거
Hamburger fullBurger = new Hamburger();
fullBurger.setBuns(2);
fullBurger.setPatties(1);
fullBurger.setLettuce(2);
fullBurger.setTomato(1);
fullBurger.setCheese(1);
fullBurger.setBacon(2);
fullBurger.setPickles(3);
fullBurger.setOnions(1);

// 치즈버거
Hamburger cheeseBurger = new Hamburger();
cheeseBurger.setBuns(2);
cheeseBurger.setPatties(1);
cheeseBurger.setLettuce(2);
cheeseBurger.setTomato(1);
cheeseBurger.setCheese(1);

// 베이컨만 있는 햄버거
Hamburger baconOnlyBurger = new Hamburger();
baconOnlyBurger.setBuns(2);
baconOnlyBurger.setBacon(2);
```
### 빌더 패턴(Builder Pattern)
