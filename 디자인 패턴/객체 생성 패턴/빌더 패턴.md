...
- **코드 복잡성 증가**
    ...
    - **설명**: 빌더 패턴을 사용하면, 각 객체 생성을 위한 별도의 빌더 클래스가 필요합니다. 이는 추가적인 코드 작성을 요구하며, 간단한 객체의 경우에는 과도한 구조를 초래할 수 있습니다. 빌더 클래스 자체도 유지보수해야 할 코드의 양을 증가시킵니다.
- **성능 저하**
    
    - **설명**: 빌더 패턴은 객체를 생성하기 전에 여러 단계를 거치며, 각 단계에서 메소드 호출이 발생합니다. 이는 생성자를 직접 사용하는 것에 비해 약간의 오버헤드를 추가할 수 있습니다. 특히 생성해야 할 객체의 수가 많거나 성능이 중요한 어플리케이션에서는 이러한 성능 저하가 문제가 될 수 있습니다.
- **유연성 감소**
    
    - **설명**: 빌더 패턴은 생성 과정을 엄격하게 제어하여 객체의 불변성을 유지합니다. 이는 객체의 속성이 한 번 설정되면 변경할 수 없다는 것을 의미합니다. 일부 경우에는 이러한 불변성이 유연성을 제한할 수 있으며, 동적으로 속성을 변경해야 하는 경우에는 적합하지 않을 수 있습니다.
