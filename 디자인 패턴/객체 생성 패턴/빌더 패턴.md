...
그런데 만약 치즈가 빠져버려 실제로는 setCheese 메서드를 실행시키지 않았다면, 생성된 cheeseBurger를 사용하는 입장에서는 알 방도가 없다. 마찬가지로 누가 치즈 대신 다른걸 추가해서 cheeseBurger이라고 인스턴스를 생성해 두었을 경우에도 사용하는 입장에서는 알 방도가 없다. 즉, 인스턴스의 일관성과 완전성이 보장되지 않는 것이다. 
